...
func main() {
	...
	for i := 0; i < Workers; i++ {
		go func(id int) {
			...
			for {
				...
				// 3. Ultra-fast Write (No fmt.Fprintf)
	...
	for {
		...
		if elapsed > 0 {
			fmt.Printf("\rPPS: %.0f | Hits: %d | Misses: %d | Streak: %d | CPU Cores: %d",
				float64(h+m)/elapsed, h, m, s, runtime.NumCPU())
		}
	
if s >= 15000 {
    state.Ready.Store(false)
    fmt.Printf("\n[!] SATURATION DETECTED - Shifting to any new relay...\n")

    // 1. Set location to 'any' to let Mullvad choose a fresh peer globally
    _ = exec.Command("mullvad", "relay", "set", "location", "any").Run()
    
    // 2. Refresh the WireGuard keys
    _ = exec.Command("mullvad", "tunnel", "set", "rotate-key").Run()
    
    // 3. Reconnect to apply the 'any' logic and new key
    _ = exec.Command("mullvad", "reconnect").Run()

    // Give the tunnel a moment to establish the new route
